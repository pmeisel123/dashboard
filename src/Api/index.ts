...
const username = __API_USERNAME__;
...
export const getTicketsApi = async(search: string ) =>  {
	const main_url = '/rest/api/3/search/jql?maxResults=5000&validateQuery=1&fields=key,assignee,creator,status,summary,updated,created,parent,timeoriginalestimate,timeestimate,timespent&jql=' + encodeURI(search);
	let last = false;
	...
	while(!last) {
		...
		const ajax_result = await response.json(); // Await the parsing of the response body as JSON
